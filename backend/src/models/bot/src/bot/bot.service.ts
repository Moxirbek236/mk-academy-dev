...
import { ExamType, LeadRequest, StudentResult } from '@prisma/client';
...
export class BotService implements OnModuleInit {
  ...
  buildResultsInlineKeyboard(results: StudentResult[]) {
  ...
  buildResultLinkKeyboard(result: StudentResult) {
  ...
  formatResultMessage(result: StudentResult): string {
    return [
  ...
  formatLeadsMessage(leads: LeadRequest[]): string {
    if (leads.length === 0) {
  ...
  formatExamType(examType: ExamType): string {
    return examType === ExamType.CEFR ? 'CEFR' : 'IELTS';
  }
  ...
  getStartAdminMessage(): string {
    return [
      'Xush kelibsiz, admin.',
      '',
      "Siz uchun boshqaruv bo'limi ochildi ⚙️",
    ].join('\n');
  }
  ...
  formatResultCreatedMessage(result: StudentResult): string {
    return [

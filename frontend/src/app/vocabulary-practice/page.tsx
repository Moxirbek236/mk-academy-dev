...
export default function VocabularyPracticePage() {
  ...
  if (words.length === 0) return (
    <div className="flex flex-col items-center justify-center p-20">
      ...
      <button onClick={() => router.back()} className="mt-6 text-[#3D855A] font-black text-[10px] uppercase tracking-widest">Orqaga qaytish</button>
    </div>
  );

  if (finished) return (
    <div className="flex flex-col items-center justify-center p-20 text-center animate-in fade-in zoom-in duration-500">
      <div className="w-24 h-24 bg-emerald-50 text-[#3D855A] rounded-[32px] flex items-center justify-center mb-6">
      ...
      <div className="flex flex-col gap-4 w-full max-w-[240px]">
        ...
        <button 
          onClick={() => router.push('/')}
          className="bg-white border border-gray-100 text-[#3D855A] py-4 rounded-2xl font-black text-xs tracking-widest uppercase active:scale-95 transition-all"
        >
          Dashboardga qaytish
        </button>
      </div>
  ...
  return (
    <div className="max-w-md mx-auto h-[80vh] flex flex-col pt-10 px-4">
      <div className="flex items-center justify-between mb-12">
        ...
        <div className="flex items-center gap-2">
          <div className="h-1.5 w-24 bg-gray-100 rounded-full overflow-hidden">
            <div 
              className="h-full bg-[#3D855A] rounded-full transition-all duration-500" 
      ...
      <div className="flex-1 flex flex-col">
        <div 
          className="bg-white rounded-[48px] p-10 border border-gray-100 shadow-xl shadow-emerald-900/5 flex flex-col items-center justify-center text-center relative overflow-hidden min-h-[360px] transition-all duration-500 group"
          onClick={() => setShowTranslation(!showTranslation)}
        >
          <div className="absolute top-0 left-0 w-full h-2 bg-[#3D855A]/10" />
          
          <div className="mb-8">
            <div className="w-16 h-16 bg-[#F2F8F5] text-[#3D855A] rounded-2xl flex items-center justify-center mb-6 mx-auto group-hover:scale-110 transition-all">
          ...
          <div className={`transition-all duration-500 flex flex-col items-center ${showTranslation ? 'opacity-100 transform-none' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
            ...
            <h3 className="text-2xl font-black text-[#3D855A] tracking-tight mb-4">{word.translation}</h3>
          ...
          <button className="absolute top-6 right-6 p-2 rounded-lg text-gray-200 hover:text-[#3D855A] transition-colors">
            <Volume2 size={20} />
          </button>
        </div>
        ...
        <div className="mt-10 grid grid-cols-2 gap-4 pb-10">
          ...
          <button 
             ...
             className="bg-emerald-50 text-[#3D855A] p-6 rounded-[32px] flex flex-col items-center gap-2 border border-emerald-100 active:scale-95 transition-all group"
          >

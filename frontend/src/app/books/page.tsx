...
import { ExternalLink, Loader2, PlusCircle, Save, SearchIcon, Upload, X } from 'lucide-react';
import { useCallback, useEffect, useMemo, useState } from 'react';
import Link from 'next/link';
import { CEFR_LEVELS, createBook, listBooks, type CefrLevel } from '@/lib/backend-api';
...
export default function Books() {
  ...
  const levels = useMemo(
    () => [{ id: 'all' as const, name: 'Barchasi' }, ...CEFR_LEVELS.map((level) => ({ id: level, name: level }))],
    [],
  );
  ...
  async function handleCreateBook() {
    ...
    try {
      ...
      setMutationError(bookError instanceof Error ? bookError.message : "Kitobni qo'shib bo'lmadi");
    } finally {
      setCreating(false);
    }
  }
  ...
  return (
    <div className="app-page pb-nav-safe pt-4 animate-in fade-in slide-in-from-bottom-4 duration-500 sm:pt-6">
        ...
        <div className="mt-2 grid grid-cols-2 gap-4 sm:grid-cols-3 sm:gap-5 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
          {books.map((book) => (
            ...
            >
              <div className="relative aspect-[3/4] w-full overflow-hidden bg-gray-100">
                ...
                {book.cefrLevel ? (
                  <div className="absolute left-3 top-3 rounded-xl border border-gray-100 bg-white/95 px-2.5 py-1 shadow-lg ring-4 ring-white/10 backdrop-blur-md sm:left-4 sm:top-4">
                    <span className="text-[9px] font-black uppercase tracking-widest text-[#2563eb]">{book.cefrLevel}</span>
                  </div>
                ) : null}
              ...
              <div className="flex flex-col gap-1 p-4 sm:p-5">
                ...
                <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400">{book.author || 'MK Academy'}</p>
                <div className="mt-3 flex items-center justify-between sm:mt-4">
                  <span className="text-[10px] font-black uppercase tracking-widest text-gray-300">PDF</span>
          ...
          {books.length === 0 ? (
            <div className="col-span-full rounded-[24px] border-2 border-dashed border-gray-100 bg-gray-50 py-12 text-center sm:rounded-[32px] sm:py-20">
              <p className="text-[12px] font-black uppercase tracking-[0.3em] text-gray-300">Hozircha kitoblar mavjud emas</p>
            </div>
          ) : null}
        </div>
      )}
      ...
      {createOpen ? (
        <div className="fixed inset-0 z-[90] flex items-end justify-center bg-black/45 px-3 backdrop-blur-sm sm:items-center sm:px-4">
          <div className="w-full max-w-lg rounded-t-[30px] border border-gray-100 bg-white p-5 shadow-2xl sm:rounded-[28px] sm:p-6">
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                <h3 className="text-xl font-black tracking-tight text-gray-900">Yangi kitob</h3>
                <p className="mt-1 text-[10px] font-black uppercase tracking-widest text-gray-400">PDF/DOC fayl majburiy</p>
              </div>
              <button
                onClick={() => {
                  setCreateOpen(false);
                ...
                className="rounded-[14px] bg-gray-50 p-3 text-gray-400 transition-all active:scale-95"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="space-y-3">
              <input
                ...
                className="w-full rounded-[16px] border border-gray-100 bg-gray-50 px-4 py-3 text-sm font-semibold outline-none transition-all focus:border-[#2563eb]"
              />
              <input
                value={form.author}
              ...
              <textarea
                ...
                className="w-full resize-none rounded-[16px] border border-gray-100 bg-gray-50 px-4 py-3 text-sm font-semibold outline-none transition-all focus:border-[#2563eb]"
              />
              <select
              ...
              <label className="flex cursor-pointer items-center gap-3 rounded-[16px] border border-dashed border-gray-200 bg-gray-50 px-4 py-3 text-sm font-bold text-gray-500">
                <Upload size={16} className="text-[#2563eb]" />
                <span className="min-w-0 flex-1 truncate">{form.coverImage?.name || 'Cover image tanlash'}</span>
                <input
                  type="file"
                  accept="image/png,image/jpeg,image/webp"
                  className="hidden"
                  onChange={(event) => setForm((current) => ({ ...current, coverImage: event.target.files?.[0] ?? null }))}
                />
              </label>

              <label className="flex cursor-pointer items-center gap-3 rounded-[16px] border border-dashed border-[#2563eb]/30 bg-blue-50 px-4 py-3 text-sm font-bold text-gray-700">
                <Upload size={16} className="text-[#2563eb]" />
                <span className="min-w-0 flex-1 truncate">{form.bookFile?.name || 'Kitob faylini tanlash'}</span>
                <input
                  type="file"
                  accept=".pdf,.zip,.doc,.docx,.xls,.xlsx,application/pdf"
                  className="hidden"
                  onChange={(event) => setForm((current) => ({ ...current, bookFile: event.target.files?.[0] ?? null }))}
                />
              </label>
            </div>
            ...
            <div className="mt-5 flex flex-col gap-3 sm:flex-row">
              <button
                ...
                className="flex-1 rounded-[16px] border border-gray-100 px-4 py-3 text-[11px] font-black uppercase tracking-widest text-gray-500"
              >
                Bekor qilish
              </button>
              <button
                ...
                disabled={creating}
                className="flex flex-1 items-center justify-center gap-2 rounded-[16px] bg-[#2563eb] px-4 py-3 text-[11px] font-black uppercase tracking-widest text-white disabled:opacity-60"
              >

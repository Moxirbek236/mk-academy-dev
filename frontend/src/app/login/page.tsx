...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = getUserFriendlyErrorMessage(normalizedError, copy.invalidCredentials);
      ...
      <div className="min-h-screen-safe flex items-center justify-center bg-[var(--app-bg)] px-safe text-[var(--app-text)]">
        <div className="flex flex-col items-center gap-4 text-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-[var(--app-primary)] shadow-xl shadow-[var(--app-primary)]/20">
            <Loader2 size={30} className="animate-spin text-[var(--primary-foreground)]" />
          </div>
  ...
  return (
    <div className="min-h-screen-safe relative flex items-start justify-center overflow-hidden bg-[var(--app-bg)] px-safe pb-safe pt-safe text-[var(--app-text)] sm:items-center sm:p-6">
      {!nativeApp && (
        <>
      ...
      <motion.button
        ...
        className="app-touch absolute left-[max(0.75rem,env(safe-area-inset-left))] top-[calc(0.75rem+env(safe-area-inset-top))] flex min-h-10 items-center gap-2 rounded-full border border-[var(--app-border)] bg-[var(--app-surface)] px-3.5 py-2 text-[10px] font-black uppercase tracking-widest text-[var(--app-muted)] shadow-sm transition-all hover:text-[var(--app-primary)] sm:left-8 sm:top-8"
      >
      ...
      <motion.div
        ...
        className="relative z-10 mt-14 w-full max-w-[28rem] rounded-[32px] border border-[var(--app-border)] bg-[var(--app-surface)] p-5 shadow-[var(--shadow-premium)] sm:mt-0 sm:p-8"
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-2">
            ...
            <div className="group relative">
              <Phone className="absolute left-5 top-1/2 -translate-y-1/2 text-[var(--app-muted)] transition-colors group-focus-within:text-[var(--app-primary)]" size={20} />
              <input
                required
                type="tel"
                autoComplete="tel"
          ...
          <div className="space-y-2">
            <div className="ml-1 flex items-center justify-between">
              ...
              <button type="button" className="text-[10px] font-black uppercase tracking-tight text-[var(--app-primary)] hover:underline">
                {copy.forgot}
              </button>
            </div>
            <div className="group relative">
              <Lock className="absolute left-5 top-1/2 -translate-y-1/2 text-[var(--app-muted)] transition-colors group-focus-within:text-[var(--app-primary)]" size={20} />
              <input
                required
        ...
        <div className="mt-8 flex flex-col items-center gap-5 sm:mt-10 sm:gap-6">
          ...
          <button className="group flex w-full items-center justify-center gap-4 rounded-[20px] border border-[var(--app-border)] bg-[var(--color-input-background)] py-3.5 transition-all hover:border-[var(--app-primary)]/20 hover:bg-[var(--app-surface)]">
            <Globe size={20} className="text-[var(--app-muted)] group-hover:text-[var(--app-primary)]" />
            <span className="text-sm font-black tracking-tight">{copy.continueWithGoogle}</span>
          </button>
